...
public class NumUtil {
	...
	public static float add(float v1, float v2,int scale) {
		BigDecimal b1 = new BigDecimal(Float.toString(v1));
		BigDecimal b2 = new BigDecimal(Float.toString(v2));
		return b1.add(b2).setScale(scale, BigDecimal.ROUND_DOWN).floatValue();
	}
	...
	public static float sub(float v1, float v2,int scale) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));
		BigDecimal b2 = new BigDecimal(Double.toString(v2));
		return b1.subtract(b2).setScale(scale, BigDecimal.ROUND_DOWN).floatValue();
	}
	 ...
	 * @return 两个参数的差
	 */
	public static float sub(float v1, float v2, float v3) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));
		BigDecimal b2 = new BigDecimal(Double.toString(v2));
		BigDecimal b3 = new BigDecimal(Double.toString(v3));
	 ...
	 * @return 两个参数的差
	 */
	public static float sub(float v1, float v2, float v3, int scale) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));
		BigDecimal b2 = new BigDecimal(Double.toString(v2));
		BigDecimal b3 = new BigDecimal(Double.toString(v3));
		return b1.subtract(b2).setScale(scale, BigDecimal.ROUND_DOWN).subtract(b3).setScale(scale, BigDecimal.ROUND_DOWN).floatValue();
	}
	...
	/**
	 * 提供（相对）精确的除法运算。当发生除不尽的情况时，由scale参数指 定精度，以后的数字不四舍五入。
	 * 
	 * @param v1
	 *            被除数
	 * @param v2
	 ...
	 * @return 两个参数的商
	 */
	public static double divN(double v1, double v2, int scale) {
		if (scale < 0) {
			throw new IllegalArgumentException("The scale must be a positive integer or zero");
		}
		BigDecimal b1 = new BigDecimal(Double.toString(v1));
